...
| 2 | マッチング | **あいことば（接続コード）を発行して相手に直接渡す**方式。マッチングサーバーも中継サーバーも作らない |
| 3 | あいことばの渡し方 | 画面に**コピーボタン**を出すだけ。QRコードの表示・読み取りは作らない。おとながLINE・メール等で渡す前提 |
| 4 | 対戦できる範囲 | 同じWi-Fi内は確実に。インターネット越しも「つながればできる」。つながらない場合は諦めて案内する（TURN中継は用意しない） |
...
### 「サーバーなし」で本当に成立する範囲（重要）

発注者の選択は「完全にサーバーなし（コードをコピペ交換）」だが、
**インターネット越しに対戦する場合だけは STUN サーバーが1つ必要**になる。
これは技術上どうしても避けられないので、区別して記録しておく。

| 種類 | 本設計で使うか | 何をするものか |
|---|---|---|
| マッチングサーバー | **使わない** | 本来「ID→相手」を結びつける役。あいことばの手渡しで代替する |
| シグナリングサーバー | **使わない** | 本来 接続情報を中継する役。あいことばの手渡しで代替する |
| STUN サーバー | **使う（インターネット越しのときだけ）** | 自分のグローバルIPと穴あきポートを教えてもらうだけ。**対戦データは一切通らない**。無料の公開STUN（Google等）を利用 |
| TURN サーバー | **使わない** | 直接つながらない相手同士のデータを丸ごと中継する役。有料になるので用意しない |

- **同じWi-Fi内**なら STUN も無しでつながる（お互いのローカルアドレスだけで足りる）
- **インターネット越し**は STUN が要る。それでも回線によっては直接つながらないことがあり、
  その場合は TURN が無いので**接続できない**。「おなじ おうちの Wi-Fi で ためしてね」と案内して終わる
- したがって「サーバーなし」＝**こちらで運用するサーバーは無い**、という意味になる
...
└── src/
    ...
    │   ├── codec.ts        # あいことばの符号化・復号
    │   ├── PeerLink.ts     # WebRTC接続とデータチャネル
    │   ├── protocol.ts     # メッセージ型・絵の分割送受信・受信検証
...
- 相手から届いた `CharacterDoc` は信用しない。§13.4.1 の上限で必ず検証する
...
- 自前のサーバー・アカウント・ランキング（P2P対戦は13章で扱うが、サーバーは持たない）
- 見知らぬ相手との自動マッチング（あいことばを知っている相手とだけ繋がる）
- TURN中継（直接つながらない回線同士の対戦は諦める）
- 3人以上の対戦・観戦
- 対戦成績の記録
- BGM（効果音のみ）
- 多言語対応（日本語ひらがなのみ）
...
### 13.3 接続方式（あいことば式 WebRTC）

#### 13.3.1 なぜこの形になるか

GitHub Pages は静的ファイルを配るだけでプログラムを動かせない。
WebRTC で直接つなぐには、お互いの接続情報（SDP＝どこに繋げばいいかの案内状）を
**交換する手段**が要る。ふつうはそこにサーバーを置くが、本設計では
**その案内状そのものを「あいことば」という文字列にして、人が手で渡す**ことで代替する。

#### 13.3.2 接続手順

```mermaid
sequenceDiagram
    participant A as ホスト（つくるひと）
    participant B as ゲスト（もらうひと）
    A->>A: あいことば① を作る<br/>(offer作成 + 接続候補あつめ)
    A-->>B: あいことば① を渡す<br/>（LINE・メール・口頭以外の手段）
    B->>B: あいことば① を貼る<br/>→ あいことば② を作る (answer)
    B-->>A: あいことば② を渡す
    A->>A: あいことば② を貼る
    A->>B: P2P接続（データチャネル）確立
    A->>B: hello（自分の絵）
    B->>A: hello（自分の絵）
```

**2往復のコピペが必要**なのがこの方式の代償。3歳児には無理なので、
マッチング画面の文言は**おとなに向けて**書く（この画面だけ漢字を許容するかは §13.12 で確認）。

#### 13.3.3 あいことばの中身

```
RKG1.<base64url( gzip( JSON.stringify({ t:'offer'|'answer', sdp }) ) )>
```

| 項目 | 決めごと |
|---|---|
| プレフィックス | `RKG1.` 固定。これが無い文字列は貼っても弾く（別バージョン・無関係な文字列の誤貼り防止） |
| 圧縮 | `CompressionStream('gzip')`。SDPは同じ語の繰り返しが多く 1/3 程度に縮む |
| 圧縮が無い環境 | `RKG1U.` プレフィックスで無圧縮 base64url にフォールバック |
| 想定長 | 圧縮ありで **700〜1200文字**程度。短くはないが、コピーボタンで扱う分には問題ない |
| 接続候補 | UDP の host / srflx のみ採用。TCP候補は落として短くする |
| 収集の打ち切り | `icegatheringstate === 'complete'` を待つが、**5秒でタイマー打ち切り**してその時点の候補で作る（待ち続ける回線があるため） |

#### 13.3.4 ICEサーバー設定

```ts
const ICE_SERVERS: RTCIceServer[] = [
  // 自分のグローバルな居場所を知るためだけに使う。対戦データは通らない
  { urls: ['stun:stun.l.google.com:19302', 'stun:stun.cloudflare.com:3478'] },
];
```

- STUN が両方落ちていても、**同じWi-Fi内なら**ローカル候補だけでつながる
- ブラウザはローカルIPを `.local`（mDNS）に隠すが、同一LAN上の2台なら相互に解決できる

#### 13.3.5 つながらなかったときの扱い

| 状況 | 判定方法 | 画面表示 |
|---|---|---|
| あいことばが壊れている | プレフィックス不一致・復号失敗 | 「あいことばが ちがうみたい。もういちど はりつけてね」 |
| 30秒たっても接続しない | `connectionState` が `connected` にならない | 「つながらなかったよ。おなじ おうちの Wi-Fi で ためしてね」 |
| 途中で切れた | `connectionState: failed/disconnected/closed` または `datachannel close` | 「あいてと はなれちゃった」→ メインメニューへ |
| 相手が無応答 | ping に15秒返らない | 同上 |
...
| メッセージ | 中身 | 送るタイミング |
|---|---|---|
| `hello` | `{ v, size, chunks, name? }` | 接続直後。絵の本体はこの後 `chunk` で送る |
| `chunk` | `{ i, data }` | 絵（gzip+base64）を16KBずつ分割して送る |
...
- **絵の分割送信**が必要な理由: 手描き1体の `CharacterDoc` は 30〜150KB。
  データチャネルの1メッセージ上限（実装により64KB前後）を超えうるので16KBに割る
...
| 段階 | 表示するもの |
|---|---|
| `role` | 「あいことばを つくる」「あいことばを もらう」の2つの大きなボタン、「やめる」 |
| `making` | 「あいことばを つくってるよ…」＋スピナー（ICE収集中） |
| `hostShare` | あいことば①（読み取り専用テキスト）＋**コピー**ボタン／相手のあいことば②の貼り付け欄＋「つなぐ」 |
| `guestPaste` | あいことば①の貼り付け欄＋「つなぐ」 |
| `guestShare` | あいことば②（読み取り専用テキスト）＋**コピー**ボタン＋「あいてに おくってね」 |
| `connecting` | 「つないでいるよ…」＋30秒でタイムアウト |
| `exchanging` | 「あいての えを もらっているよ…」＋進捗（chunk受信中） |
...
- 相手について出すのは**絵だけ**。星もステータス数値も属性バッジも出さない
- 相手が殿堂入りで名前をつけていれば、その名前を絵の下に出す（→ §13.12 で確認）
...
| 2回目以降 | 前回の選択は**残さない**（毎回まっさらから選ぶ）→ §13.12 で確認 |
...
src/
├── net/
│   ├── codec.ts        # あいことばの符号化・復号（gzip + base64url + プレフィックス）
│   ├── PeerLink.ts     # RTCPeerConnection とデータチャネルの薄いラッパ
│   ├── protocol.ts     # メッセージ型・受信バリデーション・絵の分割送受信
...
### 13.12 バランス上の懸念（実装前に試算すること）

3ターンしかないので、PvEより**1発の重みが大きい**。素の値のまま戦わせると
次の事故が起きうるので、実装フェーズで実エンジンを使って試算する。
...
**試算方法**: `simulatePvp` にシード付きRNGを注入し、
「ちいさい絵 vs おおきい絵」「同じくらいの絵どうし」で各30,000回まわして
勝率・ひきわけ率・1発で沈む確率を出す。§8.4 と同じやり方。

対処が要る場合の候補（発注者に確認してから入れる）:

- オンライン限定で会心倍率を 2 → 1.5 に下げる
- オンライン限定で体力を底上げする（例: `maxHp × 1.5`）
- 属性倍率を 2 / 0.5 → 1.5 / 0.75 に寄せる

いずれも**PvEには影響させない**（オンライン用の定数として `pvpEngine.ts` に置く）。
...
| `matchGuest` | `あいことばを もらう` |
| `matchCopy` | `📋 コピーする` |
| `matchCopied` | `コピーしたよ！` |
| `matchPastePrompt` | `あいての あいことばを はりつけてね` |
| `matchConnect` | `つなぐ` |
| `matchMaking` | `あいことばを つくってるよ…` |
| `matchConnecting` | `つないでいるよ…` |
| `matchBadCode` | `あいことばが ちがうみたい` |
...
**マッチング画面だけは、おとなが操作する前提**なので
「コピーして あいてに おくってね」のような説明が要る。ここも全部ひらがなにするか、
この画面だけ漢字を許すかは §13.16 で確認する。

### 13.15 実装フェーズと受け入れ基準

| # | フェーズ | 内容 | 受け入れ基準 |
|---|---|---|---|
| 1 | あいことばの符号化 | `net/codec.ts`。gzip + base64url + プレフィックス、無圧縮フォールバック | 往復テスト（encode→decode で元に戻る）。壊れた文字列を弾く単体テストが通る |
| 2 | P2P接続 | `net/PeerLink.ts`。offer/answer 生成、ICE収集の5秒打ち切り、状態通知 | 同一PCの2タブ間で接続でき、文字列を送り合える |
| 3 | プロトコル | `net/protocol.ts`。メッセージ型・絵の分割送受信・受信バリデーション | 上限を超える絵を送りつけても落ちずに切断する |
...
| 6 | マッチング画面 | `MatchScene`。役割えらび〜相互プレビュー | 2ブラウザで「あいことば手渡し→接続→両者OK」が通る |
...
| 10 | バランス確認 | §13.12 の試算 | 一撃死の確率と勝率の偏りを数字で確認し、必要なら定数を調整 |

**テストのしやすさ**: あいことばを人が手で渡す方式なので、E2Eでは
Playwright の2つのブラウザコンテキストを開き、
**片方のテキストエリアから読んで、もう片方に打ち込む**だけで通しテストができる。
サーバーを立てる方式より自動テストは書きやすい。

### 13.16 発注者に確認したいこと（実装前）

答えによって作りが変わるので、実装に進む前に確認したい。

| # | 論点 | こちらの案 |
|---|---|---|
| 1 | **STUNサーバーの使用可否** | インターネット越し対戦には必須。「こちらで運用するサーバーは無い」という意味では“サーバーなし”を満たす。使ってよいか。ダメなら**同じWi-Fi内限定**になる |
| 2 | マッチング画面の文言 | 2往復のコピペはおとなの作業。この画面だけ漢字を使って分かりやすくするか、全部ひらがなで通すか |
| 3 | 相手の名前の表示 | 殿堂入りで名前をつけていたら、その名前を絵の下と勝敗表示に出したい（「ぽちまるの かち！」）。出さないなら「あいて」表記 |
| 4 | 必殺技 | オンラインには入れない想定（3ターン固定なので発動条件が作れない）。これでよいか |
| 5 | 2回目以降の手 | 「もういちど」のとき、前回選んだ手を残して出すか、毎回まっさらにするか |
| 6 | 決定後のやり直し | 「けってい」を押したあと、相手待ちの間に選び直せるようにするか（現案: できない） |
| 7 | ひきわけの連続 | ひきわけたとき、そのまま「もういちど」でよいか。決着がつくまで自動で続ける仕組みは要らないか |
| 8 | オンライン中のセーブ | 対戦中に相手からもらった絵は保存しない想定。自分の絵は普段どおり保存済み。これでよいか |
| 9 | 入口の位置 | プレビュー画面のボタンは4つ（えをなおす／たたかいへ／オンラインたいせん）になる。メインメニューからも入れるようにするか |
| 10 | バランス調整の可否 | §13.12 で一撃死が多すぎた場合、オンライン限定で会心倍率や体力を調整してよいか |
| 11 | 対戦の記録 | 何勝何敗といった記録は残さない想定（毎回まっさら）。これでよいか |
| 12 | 3台以上 | 2人対戦のみ。観戦や3人以上は作らない想定でよいか |
