...
| メッセージ | 中身 | 送るタイミング |
|---|---|---|
| `hello` | `{ v, doc, name? }` | 接続直後。`doc` は絵を gzip+base64url にした文字列（§13.4.2） |
...
- 相手が殿堂入りで名前をつけていれば、その名前を絵の下に出す（→ §13.16 で確認）
...
| # | 論点 | こちらの案 |
|---|---|---|
| 1 | **ブローカーが落ちていたときの代替手段** | 無料の公開サーバーなので、止まるとオンライン対戦が丸ごと使えなくなる。前案の「接続情報を手でコピペ」を**予備の入口として残すか**、PeerJS一本でいくか（現案: 一本。作りが半分で済む） |
| 2 | あいことばの文字数 | 6文字（約8.8億通り）で提案。読み上げやすさを優先するなら4文字（約92万通り）も可能だが、当てられる可能性が上がる |
...
| 9 | 入口の位置 | プレビュー画面のボタンは3つ（えをなおす／たたかいへ／オンラインたいせん）になる。メインメニューからも入れるようにするか |
...
| 13 | 外部サービスへの接続 | オンライン対戦のときだけ PeerJS のブローカー（`0.peerjs.com`）に接続する。ブローカーには**あいことばとIPアドレス**が見え、**絵や対戦データは通らない**。この前提でよいか |
