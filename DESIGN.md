...
両者とも「つながらなかったよ」になった（§11-22）。自前のTURNを足したいときは、
既定値を消さないよう `util.defaultConfig.iceServers` に足す形で書くこと。
...
失敗したときは、文言の下に `ice:failed/failed` のような**短い印**を小さく出す。
`RTCPeerConnection` の状態をそのまま書いたもので、経路が見つからなかったのか、
候補は集まったが疎通しなかったのかを区別できる。うまくいかない環境を報告してもらうため。
