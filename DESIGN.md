...
| 11 | 見せる情報 | 体力・こうげき・はやさの**数値は隠す**。**防御属性（グーチョキパー）はおたがい見せる**（戦略画面の図で示す）※当初は防御属性も隠す決定だったが、読み合いが成立しないため第4版の改修で変更（§13.13） |
...
## 0.4 改定内容（第5版：UIを Material 3 Expressive に統一）

第4版までは画面ごとに色と影を都度決めていたため、
同じ意味のものが画面ごとに違う見た目になっていた
（進むボタンが橙だったり緑だったり、カードの角丸が14/16/18pxで揃っていない等）。
第5版では **Material 3 Expressive** のデザインシステムに一本化した。

| # | 改定項目 | 確定した仕様 |
|---|---|---|
| 1 | 色 | 生の色をやめ、**M3のカラーロール**（primary / secondary / tertiary / error と各 container、surface の階層）だけで指定する。ソースカラーは元の橙 |
| 2 | 面 | 背景は**ニュートラルなサーフェス**にし、色はボタン・チップ・カードの側で出す。第4版までの「空のグラデーション＋白抜き文字」はやめる |
| 3 | 文字 | display / headline / title / body / label の**タイプスケール**から選ぶ。3歳児向けにウェイトは1〜2段太くする |
| 4 | かたち | 角丸は**シェイプスケール**（4/8/12/16/28/48/full）から選ぶ |
| 5 | 押した感じ | 「下に濃い影を置いて沈む」表現をやめ、**シェイプモーフ（丸→角ばる）＋ステートレイヤー＋わずかな縮小**にする |
| 6 | 動き | イージングと時間を**モーショントークン**に統一する（emphasized 系＋弾むばね） |
| 7 | ボタン | filled / filled tonal / outlined / text / elevated の**強さの階層**で選ぶ。1画面の主役は1つだけ |
| 8 | 赤の意味 | error（赤）は**壊す・やめる**だけに使う。「たたかう！」は主役なので primary に変更 |
| 9 | ダークテーマ | **入れない**。3Dステージ・記念写真・ルーレットが明るい絵で固定されているため、面だけ暗くしても釣り合わない |
| 10 | 絵そのもの | 3Dステージの空と草原、ルーレットの盤、記念写真、オエカキのパレット12色は**ゲームの画作り**なので変えない |

**変えていないこと**: 画面の数・遷移・遊びのルール・文言。見た目と手ざわりだけの改定。
...
flowchart TD
    ...
    PREVIEW -->|えをなおす| DRAW
    PREVIEW -->|たたかいへ| LOADING[3.5 よみこみ画面]
    LOADING -->|敵の絵ができたら自動で| ROULETTE[4. 対戦相手ルーレット画面]
...
第5版で、プレビューから対戦へ進むときの**よみこみ画面**を足して12画面（§5.3.1）。
オンライン側はルールもエンジンも別なので、**13章にまとめて記述する**。

### 1.2 ゲームサイクル

1. 4ステップ（あたま→からだ→うで→あし）で絵を描く
...
- タップ対象は最小 56×56px
- 見た目は §5.0 のデザインシステム（Material 3 Expressive）に従う

### 5.0 デザインシステム（Material 3 Expressive）※第5版

`src/style.css` の先頭にトークンを置き、**すべてのスタイルはトークン経由で指定する**。
コンポーネントに生の色や生の数値を書かない。色を変えたいときはトークンだけ直せばよい。

#### 5.0.1 カラーロール

ソースカラーは第1版から使っている橙。そこから作った主役の色に、
緑（よい報せ）と青（ふつうの操作）を secondary / tertiary として割り当てている。

| ロール | 値 | 使うところ |
|---|---|---|
| primary / on-primary | `#bb5100` / `#ffffff` | その画面の主役ボタン、ロゴ、選ばれた手、必殺技FAB |
| primary-container | `#ffdbba` | 見出しの下の説明チップ、あいことばのマス、オススメの吹き出し |
| secondary / on-secondary | `#1b7a46` / `#ffffff` | 進む・決める、HPバー、選択中の道具 |
| tertiary / on-tertiary | `#0e6e9c` / `#ffffff` | ふつうの操作（トーナルボタン）、負けたときの見出し、「いまひとつ」 |
| error / on-error | `#b3261e` / `#ffffff` | **壊す・やめる・危ないことだけ**。ダメージ数字と「こうかばつぐん」 |
| surface | `#fff8f2` | 画面の地 |
| surface-container-low / container / high / highest | `#fff1e7` / `#fbe9dd` / `#f6e0d2` / `#f0d7c8` | カード → パネル → チップ → バーの溝、の順に濃くする |
| on-surface / on-surface-variant | `#241a11` / `#54443a` | 本文 / 補足 |
| outline / outline-variant | `#867466` / `#dcc7b6` | アウトラインボタンの枠 / 仕切り線 |
| **カスタム** gold-container | `#ffe08a` | 殿堂入りと強敵ボーナスだけの特別な金 |

コントラストは全ペアで **4.5:1 以上**を確認済み（仕切り線の outline-variant を除く）。

#### 5.0.2 タイプスケール

display / headline / title / body / label の5系統。
画面幅で伸縮させ（`clamp()`）、上限はM3の標準値、下限は小さいスマホでも収まる値にする。
ウェイトはM3の推奨より1〜2段太い（regular 500 / medium 700 / bold 800 / emphasized 900）。
ひらがなの丸ゴシックは細いと読み分けづらいため。

| 使うところ | スケール |
|---|---|
| タイトルロゴ、勝敗の見出し、％の数字 | display |
| 画面の見出し、実況の帯、ダイアログの文 | headline-small |
| ステータス行、ボタンのラベル（大） | title |
| 説明文、チップ | body / label-large |
| 補足、単位、持ち主のラベル | label-small |

#### 5.0.3 シェイプ

| トークン | 値 | 使うところ |
|---|---|---|
| corner-small | 8px | ボタングループの内側の角 |
| corner-medium | 12px | 絵札、入力欄、押した瞬間のアイコンボタン |
| corner-large | 16px | 小さいカード、押した瞬間のボタン |
| corner-extra-large | 28px | 大きいカード、ダイアログ、3Dステージ、FAB |
| corner-full | 999px | ボタン、チップ、色見本、HPバー |

#### 5.0.4 押した感じ（M3 Expressive のシェイプモーフ）

第4版までは「下に濃い影を置き、押すと沈む」で押せることを伝えていた。
第5版では M3 の作法に合わせ、次の3つで返事をする。

1. **シェイプモーフ**: 角丸が full → large に変わる（ばねのイージングで戻る）
2. **ステートレイヤー**: 文字色と同じ色を10%かぶせる（`::after` の膜）
3. **わずかな縮小**: `scale(0.96)`

ステートレイヤーの膜は `border-radius: inherit` で親と同じかたちに切る。
`overflow: hidden` で切ってはいけない（**はみ出す吹き出しまで消える**）。

#### 5.0.5 コンポーネントの対応表

| このアプリ | M3でいうと |
|---|---|
| `.btn` / `-primary` / `-go` / `-danger` / `-ghost` / `-text` / `-elevated` | filled tonal / filled(primary) / filled(secondary) / filled(error) / outlined / text / elevated ボタン |
| `.btn-small` / 既定 / `.btn-huge` | ボタンサイズ S(40) / M(56) / L(84) |
| `.icon-btn`、`#mute-btn` | アイコンボタン |
| `.btn-group` ＋ `.tool-btn` | 連結ボタングループ（ペン/ぬる、ふとさの3段） |
| `.m3-card`、`.stat-panel`、`.enemy-card`、`.elem-card`、`.percent-box` | カード（サーフェス階層で表す） |
| `.hint-line`、`.strong-tag`、`.effect-tag`、`.tip-badge`、`.stage-hint` | チップ／バッジ／ツールチップ |
| `.hp-bar`、`.step-dots`、`.spinner` | リニアプログレス／進行ドット／ローディング |
| `.name-input`、`.code-input` | アウトラインのテキストフィールド |
| `.dialog` | ベーシックダイアログ |
| `.special-btn` | 拡張FAB |
...
`あたま → からだ → うで（両うで）→ あし（両あし）` の順に1パーツずつ描く。

順番は `STEP_ORDER`（`paint/types.ts`）**1か所だけ**で決まる。
見出しの番号も「前の工程＝かべ」の判定も、この配列の並びから作る（§11-35）。

- 画面上部にステップインジケータ（4つの丸、現在位置ハイライト）と指示文:
  - 「①あたまを かこう！」「②からだを かこう！」「③りょううでを かこう！」「④りょうあしを かこう！」
    （①〜④の番号は文言に焼き込まず、`STEP_ORDER` の位置から付ける）
- 各ステップに**うすいガイド枠**（点線シルエット: あたま=上の円、からだ=中央の楕円、うで=左右、あし=下部左右）を表示する。ガイドはあくまで目安で、枠外に描いても良い
...
- **パーツが変わったら、道具と太さは既定（ペン・ふつう）に戻す**。
  前のパーツで「ぬる」や「ふとい」にしたまま次に進むと、次のパーツの最初のタップで
  いきなり塗りつぶしてしまう。どのパーツも同じ状態から描き始められるようにする。
  **色だけは引き継ぐ**（同じ色で描き続けたい場面が多いため）
...
第1版では、あとの工程で前の工程のパーツの内側をタップすると、そのパーツを塗りつぶせてしまう不具合があった。
塗りは現在パーツのレイヤーに書かれるが、そのレイヤーが前の工程より手前に合成されることがあるため、
見た目上 前のパーツが塗り替わってしまうため。

改定後のルールは次の3点。

1. **境界判定は全レイヤーの合成画像で行う**（第1版と同じ）。
   これにより、あたま の輪郭を「かべ」にして からだ を塗ることができる。
...
| **アクション（押せる・その場で反応）** | 3Dステージに**付随した1つの操作パネル**。4ボタンとも**同じトーナルボタン**（tertiary-container）※第5版まで青系＋沈む影 | こうげき／いたい！／よける／ぼうぎょ |
| **遷移（押せる・画面が変わる）** | 画面下部に独立配置。大きく、強さの違うボタン（もどる=アウトライン、すすむ=filled primary、オンライン=filled secondary） | えを なおす／たたかいへ すすむ！／オンラインたいせん |

具体的な変更点:

- **属性表示**: オレンジの枠付きバッジをやめ、`❤️ たいりょく` などと同じ**ステータス1行**にする。
  `✊ ぞくせい　グー（どんき）` の形式で、他のステータス行と同じ書式・同じ色に揃える
- **アクションボタン**: 個別の色分け（赤・青・緑・オレンジ）をやめ、**4つとも同一のトーナルボタン**にする。
...
| 「たたかいへ すすむ！」ボタン | **よみこみ画面**を経てルーレット画面へ（PvEの5連勝チャレンジ。§5.3.1） |
| 「オンラインたいせん」ボタン ※第4版 | 対人マッチング画面へ（§13.7）。**この画面がオンライン対戦の唯一の入口**。3つ並ぶと狭くなるため、遷移ボタンは「えをなおす／たたかいへ」を1段目、「オンラインたいせん」を2段目に置く |

#### 5.3.1 よみこみ画面（LoadingScene）※第5版で追加

敵9体ぶんの絵を展開して解析する処理に数秒かかる（実測: ソフトウェア描画の
確認環境で**約9秒**、1体あたり約1秒）。これを「たたかいへ すすむ！」を押した
直後にまとめてやると画面が固まり、3歳児には**ボタンが効いたのかどうかが分からない**。
そこで、押したらすぐ出る「よみこみちゅう」の画面を挟む。

| 要素 | 仕様 |
|---|---|
| 見出し | 「よみこみちゅう」。ゆっくり大きさが揺れる |
| バー | M3のリニアインジケータ。**本当の進み具合**（9体中いくつ終わったか）で伸びる |
| 走るキャラ | バーの先っぽを**自分が描いたキャラのあたま**が跳ねながら進む。プレビューで解析済みのものを借りるだけで、ここでは何も作らない |
| 相手の顔 | 用意できた相手が1体ずつ下に並ぶ。**字が読めなくても進み具合が分かる** |
| 最低表示時間 | 700ms。2回目以降は作り済みで一瞬終わるが、パッと切り替わると押せたのか分からないため |
| 終わったら | 自動でルーレット画面へ |

**作り方の要点は2つ。**

1. **1体ずつ作って、間に画面を描かせる。**
   `requestAnimationFrame` だけでは、続きがマイクロタスクとして
   **描画の前**に動いてしまい、重い処理が同じコマに入ってバーが一度も動かない。
   rAF のあとに `setTimeout` を挟み、描画が済んでから次の1体に取りかかる。
2. **動かす飾りは transform だけにする。**
   1体を作っている約1秒はメインスレッドが止まるので、`width` や `margin` の
   アニメーションはその間まったく進まない（実際、最初は幅のトランジションが
   効かずバーが伸びなかった）。跳ねる動きと光の流れは `transform` にして
   合成スレッド側で動かす。バーの幅は段ごとにパッと入れる。
   実測でコマの途切れは**最大157ms**で、画面は最後まで動き続けている。
...
| 25 | オンライン: **絵は必ず分割して送る** | 「gzipで収まるから1メッセージでよい」は誤り。**指で描くと点が桁違いに増え**（実測 56,000点 / 符号化後88KB）、1メッセージ上限が64KBの端末では送信が失敗する。しかも**片方向だけ黙って届かない**形で出るので追いにくい。8,000文字ずつに割り、`send()` の失敗も握りつぶさず表面化させる |
| 26 | オンライン: 実機ログを画面に出す | スマホではコンソールが見られないので、`net/netLog.ts` に通信ログを溜め、マッチング画面の「🔧 ログ」で開けるようにした。エラー時は自動で開く。状態遷移・ICE候補の内訳・送受信・`maxMessageSize` が残る |
| 27 | オンライン: **防御属性は隠さず見せる** | 「相手は絵だけ見せる」という当初の確定仕様では、グーチョキパーを当てずっぽうで選ぶだけになり読み合いが成立しなかった（3歳児は前の戦闘のタグを覚えていられない）。**戦略画面でおたがいの防御属性を図で見せる**方針に変更した。隠すのは数値だけ（§13.13） |
| 28 | オンライン: オススメ印の意味 | よく効く手に💡の吹き出しを出すが、**その手はジャンケンの手でもある**ので必勝ではない。読み取り違いを防ぐため「オススメの ても ジャンケンに まけると こうげき できないよ」を図の下に必ず出す |
| 29 | 図の色づかい | 意味を色分けすると3歳児には対応づけが覚えられず、見るべき場所が散る。**同系色（クリーム〜タン）＋墨色の文字**でそろえ、意味は**形（実線＝こうげき／破線＝まもり）と矢印の向きと一文**で運ぶ。色で目立たせるのは吹き出しひとつだけ |
| 30 | M3化: **ステートレイヤーを overflow で切らない** | 押した感じを出す膜（`::after`）を `overflow: hidden` で親のかたちに切ると、**外にはみ出す吹き出し（オススメ印）まで消える**。膜側に `border-radius: inherit` を書いて切るのが正しい |
| 31 | M3化: 赤を主役に使わない | 「たたかう！」を error（赤）で出していたが、M3では赤は**壊す・やめる**の色。主役ボタンは primary に変える。赤は確認ダイアログとダメージ表示だけに残す |
| 32 | M3化: ボタンのラベルは折り返さない | `.btn` に `white-space: nowrap` を入れる。2行になると押す場所が縦に伸び、3歳児が狙う的の形が毎回変わってしまう。入らないときは**余白と文字を1段落とし**（430px以下）、それでも入らない小さい端末（**380px以下**）では**行ごと折り返して縦に積む** |
| 33 | M3化: せまい画面の道具は7つで1段 | 連結グループ（ペン/ぬる）＋（ふとさ3つ）＋単独2つ＝7つ。430px以下では1つあたり44pxまで詰めて**1段に収める**（2段になると絵を描く面が削られる）|
| 34 | M3化: 画面遷移アニメーション中は採寸しない | 自動確認で「画面外にはみ出している」と誤検知した原因は、`scene-in`（16px下から入る）の途中で測っていたこと。**700ms待ってから測る** |
| 35 | ステップ順は STEP_ORDER 1か所で決める | 工程の順番は**見出しの番号**と**「前の工程＝かべ」の判定**の両方を決めている。文言に「①からだ」と焼き込むと順番を変えたときに必ずずれるので、番号は `stepTitleOf()` が `STEP_ORDER` の位置から付ける。初期ステップ・最終ステップも `STEP_ORDER[0]` / 末尾から取る |
| 36 | 敵の絵はステップ順に並べ直してから持つ | 敵の絵も同じ描画エンジンで再生するので、**オペの並び順で塗りの結果が変わる**。第5版で工程順を あたま→からだ に変えたとき、からだ先行で書かれていた敵の定義では、あたまの塗りが からだ の輪郭を覆ってしまった（ニンジャの黒フチが消えた）。`DocBuilder.build()` で **STEP_ORDER の順に並べ替えて seq を振り直す**ようにして、定義はパーツごとにまとめて書けるまま、順番の変更に追従できるようにした |
| 37 | 待たせる画面のアニメーションは transform だけにする | 重い処理でメインスレッドが止まっている間、`width` や `margin` のアニメーションは**1コマも進まない**。よみこみバーを幅のトランジションで伸ばしたら、止まったまま描かれて「動いていない画面」になった。跳ねる・光が流れるといった飾りは `transform`（合成スレッド側）にすれば、処理中も動き続ける |
| 38 | 画面を描かせてから重い処理に入る | `await new Promise(r => requestAnimationFrame(r))` では、続きが**描画の前**にマイクロタスクとして動くので、重い処理が同じコマに入ってしまう。`requestAnimationFrame` のあとに `setTimeout` を挟んで、描画が済んでから次に進む |
...
#### 13.4.1 絵の圧縮と、**必ず分割して送ること**

```
doc文字列 = base64url( gzip( JSON.stringify(CharacterDoc) ) )
```

圧縮したうえで、**必ず `CHUNK_CHARS`（8,000文字）ずつに割って送る**。

当初は「gzipで5〜20KBに収まるから分割不要」と判断したが、**これは誤りだった**。
マウスで描いた絵ではそうなるが、**指で描くと点の数が桁違いに増える**。
間引きは2px刻みなので、実測で **56,000点 / JSON 558KB → 符号化後 90,279文字（88KB）**
という絵が普通に作られる。

データチャネルの1メッセージ上限は端末によって違い、**64KBしかない端末もある**。
超えると `send()` が失敗するが、**片方向だけ黙って届かない**という追いにくい形で出る
（実機で「スマホは相手の絵を受け取れるのに、PCは受け取れない」という不具合になった）。

| 項目 | 決めごと |
|---|---|
| 1かたまり | 8,000文字（どの端末の上限にも余裕で収まる） |
| 手順 | `hello`（全体の文字数とかたまりの数）→ `chunk` を数のぶんだけ |
| 組み立て | 番号で穴を埋め、全部そろってから復元する（順番が入れ替わっても平気） |
| 上限 | `MAX_CHUNKS`（200こ ≒ 1.6MB）を超える `hello` は受け付けない |
| 送信失敗 | `send()` を try/catch で囲み、失敗したら**必ずエラーとして表面化させる**（黙って消さない） |

- `CompressionStream` が無い環境では無圧縮 base64url にフォールバックする
- 符号化は `net/codec.ts`、分割・組み立ては `net/protocol.ts` に置く
...
│ │  [🖐] ➡2ばい [✊] ││  [✊] ➡2ばい [✌️]  ││
│ │ あいての て   きみ ││ きみの て    あいて ││
│ │ パーを だされると  ││ グーで こうげきすると ││
│ │ ダメージ 2ばい！  ││ こうかばつぐん！    ││
│ └───────────────┘└───────────────┘ │
│  オススメの ても ジャンケンに まけると        │
│  こうげき できないよ                      │
├────────────────────────────────────────┤
│  えらんだ ては ジャンケンにも なるし…        │
│  1かいめ   [✊💡] [✌️] [🖐]                │
...
| 相手の絵 | 「あいての まもり」の見出しに小さく出す（誰と戦っているか分かるように） |
...
#### ぞくせいの かんけいの図（第4版の改修で追加）

相手の防御属性が見えないと、グーチョキパーを**当てずっぽうで選ぶだけ**になり
読み合いが成立しない。そこで**おたがいの防御属性を見せ**、
「どの手を出されると痛いか」「どの手がよく効くか」を図で示す。

| 要素 | 中身 |
|---|---|
| 左のカード | **きみの まもり**。`strongAgainst(自分の属性)` ➡ 自分の属性。「パーを だされると ダメージ 2ばい！」 |
| 右のカード | **あいての まもり**。`strongAgainst(相手の属性)` ➡ 相手の属性。「グーで こうげきすると こうかばつぐん！」 |
| 向き | どちらも **「こうげきの手 ➡ まもり」** で統一する。左右で向きを変えない |
| 絵札の下 | 誰の手かを小さく書く（`あいての て` / `きみ` / `きみの て` / `あいて`） |
| まもりの絵札 | 枠を**破線**にして、こうげきの手（実線）と役割を分ける |
| オススメ印 | 相手によく効く手に、**小さな💡と小さな「オススメ」を入れた丸い吹き出し**を添える。図の中と、3つのターン行の同じ手すべてに出す |
| 注意書き | 「オススメの ても ジャンケンに まけると こうげき できないよ」。**必勝手ではない**ことを明記する |
| 色 | **ニュートラルなサーフェス階層だけ**で面を作り、文字は on-surface。意味を色分けしない。色を持つのは吹き出し（primary-container）ひとつだけ ※第5版でトークン化。考え方は変えていない |

**なぜ色分けしないか**: 3歳児は色と意味の対応づけを覚えられない。
色を増やすほど「どこを見ればいいか」が散るので、面は同系色でそろえ、
**形（実線／破線）と位置（矢印の向き）と一文**で意味を運ぶ。

`strongAgainst(defender)` は `game/element.ts` に置く。
「守りの弱点」と「よく効く攻撃」は同じ関数で、`elementMultiplier` と同じ三すくみ表を見る
（倍率の定義が2か所に散らないようにするため）。
...
| 運の比率が高い | ジャンケンの勝ち負けは1/3ずつ。属性倍率は、当初は見えないので 1/3ずつ（×2 / ×1 / ×0.5）だったが、**第4版の改修で防御属性を見せるようにしたので、×2を狙って選べる**（§13.13） |
...
### 13.13 運の比率について（第4版の改修で方針変更）

#### 当初の考え（取り下げ）

「相手の情報は絵だけ見せる」という確定仕様の帰結として**初戦はほぼ運**になるが、
1戦終えればこうかばつぐん／いまひとつのタグから相手の防御属性が推測できるので、
「もういちど たたかう」で読み合いになる ―― と見込んでいた。
...
- 3歳児は戦闘中のタグを覚えていられない。1戦目の情報が2戦目に持ち越されない
- 3ターンしかないので、タグが出ないまま（回避・ジャンケン負け）終わることもある
- 結果として**初戦も再戦も「なんとなく選ぶだけ」**になり、選択に意味が生まれない

#### 変更後（現行）

**おたがいの防御属性を戦略画面で見せる**（§13.8 の図）。隠すのは数値だけにする。

| 隠す | 見せる |
|---|---|
| 体力・こうげき・はやさの数値、星の数、キャラ名 | 防御属性（グーチョキパー）、相手の絵 |

読み合いは**属性を隠すこと**ではなく、**手が二重の意味を持つこと**から生まれる。

- こうかばつぐんを狙って グー を出す → 相手が パー を出していたらジャンケンに負けて**攻撃できない**
- 相手も同じ図を見ているので、「こちらの弱点を突きにくる手」が読める
- つまり **「効かせにいく」か「相手の狙いを潰しにいく」か**の二択になる

一撃死のリスク（§13.12）が残っているので、
「弱点を突かれる手を避ける」ことにも意味がある。
`オススメ` は**ダメージの目安**であって必勝手ではない、と画面にも書く。
...
| 16 | 相手の防御属性 | ~~隠す~~ → **見せる**。第4版の改修で変更（§13.13）。隠したままだと当てずっぽうで手を選ぶだけになり、読み合いが成立しなかったため。数値は引き続き隠す |
